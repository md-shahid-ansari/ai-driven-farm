...
const FarmData = () => {
    ...
    useEffect(() => {
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                // Fetch weather data
                axios.get(`${URL}/weather?lat=${latitude}&lon=${longitude}`)
    ...
    const fetchRecommendations = (soil_type) => {
        axios.get(`${URL}/recommended_crops?temp=${weatherData.main?.temp}&humidity=${weatherData.main?.humidity}&soil_type=${soil_type}`)
            .then(response => {
    ...
    const fetchSeasonalCrops = (soil_type) => {
        axios.get(`${URL}/seasonal_crops?lat=${location.lat}&lon=${location.lon}&soil_type=${soil_type}`)
            .then(response => {
